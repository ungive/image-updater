...
function Server(srv, opts){
  ...
  this.serveClient(false !== opts.serveClient, opts.folder);
...
Server.prototype.serveClient = function(v, folder){
  if (!arguments.length) return this._serveClient;
  this._serveClient = v;

  //
  // Replaced require.resolve with 'node_modules/' for compatibility with nexe.
  // Moved socket.io.min.js into site/js folder. The path to that folder is passed as an argument.
  //
  if (v && !clientSource) {
    clientSource = read(folder + '/js/socket.io.min.js', 'utf-8');
    // clientSource = read('node_modules/socket.io-client/dist/socket.io.min.js', 'utf-8');
    // clientSource = read(require.resolve('socket.io-client/dist/socket.io.min.js'), 'utf-8');
    try {
      clientSourceMap = read('node_modules/socket.io-client/dist/socket.io.js.map', 'utf-8');
